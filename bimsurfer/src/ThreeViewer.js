...
export default class ThreeViewer extends EventHandler {
    ...
    containedInModel(obj) {
         for (let m of this.createdModels) {
    ...
    reset(params) {
        if (params.colors) {
            for (let [id, mat] of this.originalMaterials) {
    ...
    loadglTF(src) {
        ...
        loader.load(src + (isIE11 ? ".unoptimized" : "") + ".glb", (gltf) => {
                ...
                if (this.firstLoad) {
                    ...
                    var fovFactor = Math.tan(this.camera.fov / 2 / 180 * 3.141592653);
                    var outside = 0.;
    ...
    _mouseClick(evt) {
        ...
        var clearSelection = this.cfg.app.shouldClearSelection(evt);
    ...
    setVisibility(params) {
        let ids = params.clear ? this.allIds : params.ids;
        let visibility;
        if (params.clear) {
    ...
    setSelection(params) {
        let collection = params.secondary ? this.secondarySelected : this.selected;
        this.secondarySelected.clear();
        if (params.clear) {
            collection.clear();
        }
        params.ids.forEach((id) => {
            var id2 = this.nameToId.get(id);
            var node = this.scene.getObjectById(id2);
            if (node) {
                if (node.type === 'Group') {
                    // Handle objects with multiple materials which become groups
                    for (var c of this.scene.getObjectById(id2).children) {
                        collection.add(c.id);
                    }
                } else {
                    collection.add(id2);
    ...
    createGeometry(id, ps, ns, clrs, idxs) {
        ...
        var geometry = this.createdGeometries[id] = new THREE.BufferGeometry();
    ...
    createObject(modelId, roid, oid, objectId, geometryIds, type, matrix) {
        ...
        var mesh = new THREE.Mesh(this.createdGeometries[geometryIds[0]], material);
